...
EcsSystemHandler::EcsSystemHandler()
{
...
void EcsSystemHandler::populateECS(Game& game)
{
	...
	case packedArray:
		packedArrayManager = std::make_shared<PackedArrayManager>();
		// Use existing systems: TTL, input, movement, graphics, etc.
		logicSystems = { ttl, input, movement, gameplay, collider };
...
void EcsSystemHandler::updateECS(float elapsedTime, Game& game, SystemType _systemType)
{
	...
	case packedArray:
		setBigArraySystems(_systemType);
		updateBigArray(elapsedTime, game, packedArrayManager->getEntities());
		break;
...
void EcsSystemHandler::setBigArraySystems(SystemType _systemType)
{
	switch (_systemType)
	{
...
void EcsSystemHandler::updateBigArray(float elapsedTime, Game& game, std::vector<std::shared_ptr<Entity>> _entities)
{
...
void EcsSystemHandler::updateArchetypes(float elapsedTime, Game& game, std::vector<std::shared_ptr<Entity>> _entities, SystemType _systemType)
{
	entityIt = _entities.begin();
	while (entityIt != _entities.end())
	{
